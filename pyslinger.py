...
# Modified to add a special protocol for Qcamp (Adrian, Jun 2017)
...
class QCamp():
    ...
    def process_code(self, ircode):
        ...
        for i in ircode:
            if i == "0":
                self.construct_pulse(self.ir_zero)
            elif i == "1":
                self.construct_pulse(self.ir_one)
            else:
                print("Encounter non binary digits")
                return 1
        # Finally, send IR pulse trail
        self.construct_pulse(self.ir_trail)
        return 0
...
class IR():
    def __init__(self, gpio_pin, protocol, protocol_config):
        ...
        if protocol == "NEC":
            ...
            self.protocol = QCamp(self, **protocol_config)
        else:
